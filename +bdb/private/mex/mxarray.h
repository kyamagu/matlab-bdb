...
//
// The library provides mex::MxArray class for data conversion between mxArray*
// and C++ std types. Examples are the following.
//
...
public:
  ...
  // vector<double> values = numArray.toVector<double>();
  // @endcode
  ...
  inline std::vector<int> size() const {
    return std::vector<int>(dims(), dims() + ndims());
  }
  ...
  mwIndex subs(mwIndex row, mwIndex column = 0) const;
  // Offset from first element to desired element.
